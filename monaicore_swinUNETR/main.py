...
parser.add_argument('--logdir', default='/mnt', type=str, help='directory to save the tensorboard logs and checkpoints')
...
parser.add_argument('--space_y', default=1.5, type=float, help='spacing in y checkpointdirection')
...
def main():
    ...
    # if args.distributed:
        ...
        # print('Found total gpus on the node', args.ngpus_per_node)
...
def main_worker(args):    
    ...
    if args.distributed:
        ...
        # dist.init_process_group(backend=args.dist_backend,  # dist.init_process_group
    ...
    torch.cuda.set_device(device)         
    print(f"[{os.getpid()}] current gpu: {torch.cuda.current_device()}")
    ...
    if args.rank == 0:
        print(f"[{args.rank}] " + f"Batch size is: {args.batch_size}, max_epochs: {args.max_epochs}")
    ...
    if args.resume_ckpt:
        ...
        if args.rank == 0:
            print(f"[{args.rank}] " + f"Using pretrained weights")          
    ...
    post_pred = AsDiscrete(argmax=False, threshold=0.5)  # --logit_thresh is deprecated
    ...
    if args.checkpoint is not None:
        ...
        if 'epoch' in checkpoint:
            # start_epoch = checkpoint['epoch']   # don't need to +1?
            start_epoch = checkpoint['epoch'] + 1
        if 'best_acc' in checkpoint:
            best_acc = checkpoint['best_acc']
        if args.rank == 0:
            print(f"[{args.rank}] " + f"=> loaded checkpoint '{args.checkpoint}' (epoch {start_epoch - 1}) (best_acc {best_acc})")               
    ...
    if args.distributed:
        ...
        # For multi-device modules and CPU modules, it must be None, and the module itself dictates the output location. 
    ...
    accuracy = run_training(device=device,
                            model=model,
