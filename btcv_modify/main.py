...
parser.add_argument(
    "--ssl_pretrained_path",
    default="/mnt/ssl_pretrained/ssl_pretrained.pt",
    type=str,
    help="pretrained model name",
)
# load swinUNETR full model
...
def main_worker(args):
    ...
    # load pretrained SwinUNETR (downloaded)
    if args.resume_ckpt:
    ...
    # load pretrained ssl encoder. not loading best_val?
    if args.use_ssl_pretrained:
        try:
            ...
            if "swin_vit" in list(state_dict.keys())[0]:
                if args.rank == 0:     
                    print(f"[{args.rank}] " + "Tag 'swin_vit' found in state dict - fixing!")
                for key in list(state_dict.keys()):
    ...
    # output warining: `to_onehot=True/False` is deprecated, please use `to_onehot=num_classes` instead.        
    ...
    best_acc = 0
    start_epoch = 0
    # load checkpoint SwinUNETR (trained by this script)
